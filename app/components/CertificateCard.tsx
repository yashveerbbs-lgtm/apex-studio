...
export default function CertificateCard({ studentName, courseName, date, awardType = "Certificate of Completion" }: CertificateProps) {
  ...
  // 2. ADD TO LINKEDIN LOGIC (With Verification Support)
  const handleAddToLinkedIn = () => {
    const issueDate = new Date(date)
    const issueYear = issueDate.getFullYear()
    const issueMonth = issueDate.getMonth() + 1 
    
    // Generate a mock unique certificate ID (In production, pull this from Supabase)
    const certId = `APEX-${Math.random().toString(36).substr(2, 9).toUpperCase()}`

    const linkedInUrl = `https://www.linkedin.com/profile/add?startTask=CERTIFICATION_NAME` +
      `&name=${encodeURIComponent(courseName)}` +
      `&organizationName=${encodeURIComponent("Apex Studio")}` +
      `&issueYear=${issueYear}` +
      `&issueMonth=${issueMonth}` +
      `&certId=${certId}` +
      `&certUrl=${encodeURIComponent(`https://yourdomain.com/verify/${certId}`)}`
